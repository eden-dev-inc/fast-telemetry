...
def dumbbell_svg(rows: List[Dict[str, object]], title: str, fast_key: str, otel_key: str) -> str:
    ...
    svg = [f"<h3>{html.escape(title)}</h3>", '<div class="note">Left dot = OTel, right dot = fast-telemetry (log scale).</div>', f'<svg viewBox="0 0 {w} {h}" class="chart">']
    axis_y = h - bottom
    ...
    svg.append(
        f'<circle cx="{left+92}" cy="{legend_y}" r="5" class="fast_dot"/><text x="{left+104}" y="{legend_y+4}" class="legend">fast-telemetry</text>'
    )
...
<head>
  <meta charset="utf-8">
  <title>fast-telemetry Bench Suite Report</title>
  <style>
...
<body>
  <h1>fast-telemetry Bench Suite Report</h1>
