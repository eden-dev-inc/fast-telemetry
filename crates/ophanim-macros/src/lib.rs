//! Derive macros for ophanim.
//!
//! Provides:
//! - `#[derive(ExportMetrics)]` to auto-generate Prometheus export code
...
///
/// Supports both unlabeled metrics (`Counter`, `Gauge`, `Histogram`) and
/// labeled metrics (`LabeledCounter<L>`, `LabeledGauge<L>`, `LabeledHistogram<L>`).
///
/// # Example
///
/// ```ignore
/// use ophanim::{Counter, Histogram, Gauge, LabeledCounter, DeriveLabel};
///
...
fn derive_export_metrics_impl(input: DeriveInput) -> syn::Result<TokenStream> {
    ...
    for field in fields.iter() {
        ...
        prometheus_exports.push(quote! {
            ophanim::PrometheusExport::export_prometheus(&self.#field_name, output, #prom_metric_name, #help);
        });

        dogstatsd_exports.push(quote! {
            ophanim::DogStatsDExport::export_dogstatsd(&self.#field_name, output, #statsd_metric_name, tags);
        });

        otlp_exports.push(quote! {
            ophanim::OtlpExport::export_otlp(&self.#field_name, metrics, #prom_metric_name, #help, time_unix_nano);
        });
        ...
        match metric_kind {
            MetricKind::Counter => {
                ...
                delta_exports.push(quote! {
                    ...
                    ophanim::__macro_support::__write_dogstatsd(output, #statsd_metric_name, delta, "c", tags);
                });
            }
            MetricKind::Distribution => {
                ...
                delta_exports.push(quote! {
                    ...
                    ophanim::__macro_support::__write_dogstatsd_distribution_delta(
            ...
            MetricKind::DynamicCounter => {
                ...
                state_fields.push(quote! { #field_name: std::collections::HashMap<ophanim::DynamicLabelSet, isize>, });
                state_inits.push(quote! { #field_name: std::collections::HashMap::new(), });
                delta_exports.push(quote! {
                    let overflow = self.#field_name.overflow_count();
                    if overflow > 0 {
                        log::warn!(
                            "ophanim: {} hit cardinality cap, {} records routed to overflow",
                            #statsd_metric_name,
                            overflow
                        );
                    }
                    ...
                    self.#field_name.visit_series(|labels, current| {
                        let key = ophanim::DynamicLabelSet::from_canonical_pairs(labels);
                        current_keys.insert(key.clone());
                        let previous = state.#field_name.get(&key).copied().unwrap_or(0);
                        let delta = current - previous;
                        state.#field_name.insert(key, current);
                        ophanim::__macro_support::__write_dogstatsd_dynamic_pairs(
                            output,
                            #statsd_metric_name,
                            delta,
                            "c",
            ...
            MetricKind::DynamicDistribution => {
                ...
                state_fields.push(quote! { #buckets_state_field: std::collections::HashMap<ophanim::DynamicLabelSet, [u64; 65]>, });
                state_inits
                ...
                delta_exports.push(quote! {
                    ...
                    self.#field_name.visit_series(|labels, _count, _sum, snap| {
                        ...
                        ophanim::__macro_support::__write_dogstatsd_distribution_delta_dynamic_pairs(
            ...
            MetricKind::DynamicHistogram => {
                ...
                state_fields.push(quote! { #count_state_field: std::collections::HashMap<ophanim::DynamicLabelSet, u64>, });
                state_fields.push(quote! { #sum_state_field: std::collections::HashMap<ophanim::DynamicLabelSet, u64>, });
            ...
            MetricKind::Histogram => {
                ...
                delta_exports.push(quote! {
                    ...
                    ophanim::__macro_support::__write_dogstatsd(output, #count_metric_name, delta_count, "c", tags);
                    ophanim::__macro_support::__write_dogstatsd(output, #sum_metric_name, delta_sum, "c", tags);
                });
            }
            MetricKind::LabeledCounter(label_ty) => {
                ...
                state_inits.push(quote! {
                    #field_name: vec![0; <#label_ty as ophanim::LabelEnum>::CARDINALITY],
                });
                delta_exports.push(quote! {
                    for idx in 0..<#label_ty as ophanim::LabelEnum>::CARDINALITY {
                        let label = <#label_ty as ophanim::LabelEnum>::from_index(idx);
                        let current = self.#field_name.get(label);
                        let delta = current - state.#field_name[idx];
                        state.#field_name[idx] = current;
                        ophanim::__macro_support::__write_dogstatsd_with_label(
                            output,
                            #statsd_metric_name,
                            delta,
                            "c",
                            <#label_ty as ophanim::LabelEnum>::LABEL_NAME,
                            label.variant_name(),
                            tags,
                        );
                    }
            ...
            MetricKind::LabeledHistogram(label_ty) => {
                ...
                state_inits.push(quote! {
                    #count_state_field: vec![0; <#label_ty as ophanim::LabelEnum>::CARDINALITY],
                });
                state_inits.push(quote! {
                    #sum_state_field: vec![0; <#label_ty as ophanim::LabelEnum>::CARDINALITY],
                });
                delta_exports.push(quote! {
                    for idx in 0..<#label_ty as ophanim::LabelEnum>::CARDINALITY {
                        let label = <#label_ty as ophanim::LabelEnum>::from_index(idx);
    ...
    let otlp_method = if enable_otlp {
        quote! {
            ...
            /// Use `ophanim::otlp::now_nanos()` to get the current time.
            ///
            /// Requires the `otlp` feature on the `ophanim` dependency.
            pub fn export_otlp(&self, metrics: &mut Vec<ophanim::otlp::pb::Metric>, time_unix_nano: u64) {
                #(#otlp_exports)*
            }
        }
    } else {
    ...
    let expanded = quote! {
        ...
        impl #name {
            ...
            pub fn export_dogstatsd_with_temporality(
                ...
                output: &mut String,
                tags: &[(&str, &str)],
                temporality: ophanim::Temporality,
                state: &mut #state_name,
            ) {
                match temporality {
                    ophanim::Temporality::Cumulative => self.export_dogstatsd(output, tags),
                    ophanim::Temporality::Delta => self.export_dogstatsd_delta(output, tags, state),
                }
            }
...
fn derive_label_enum_impl(input: DeriveInput) -> syn::Result<TokenStream> {
    ...
    let expanded = quote! {
        impl ophanim::LabelEnum for #name {
