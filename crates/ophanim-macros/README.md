...
Derive macros for the `ophanim` crate.
...
```rust
use ophanim::{LabeledCounter, DeriveLabel, LabelEnum};
...
Auto-generates `export_prometheus(&self, output: &mut String)` method for metric structs.

```rust
use ophanim::{Counter, Gauge, Histogram, ExportMetrics};
...
| `#[otlp]` | struct | No | Generate `export_otlp()` method (requires `ophanim/otlp` feature) |
